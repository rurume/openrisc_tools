...
/************ Added by iceobelisk ***********/
void handle_sim_command_simplified(char *cmd) {
	char *redirstr;
	int argc;
	char *argv[5];
	char *cur_arg;
	...
	* breaking the sim when the user doesn't want it to break). */
	...
	cur_arg = (char*)malloc(strlen(cmd) * sizeof(char));
	strcpy(cur_arg, cmd);
	cur_arg = strip_space(cur_arg);
	if(*cur_arg == '\n') strcpy(cur_arg, prev_str);
	else strcpy (prev_str, cur_arg);

	if((redirstr = strchr(cur_arg, '>'))) {
		redirstr = strip_space(++redirstr);

		while(!isspace((int)*redirstr) && *redirstr) redirstr++;
		*redirstr = '\0';
	...
	if(*cur_arg) {
  		...
  		while(*cur_arg) {
			argv[argc] = cur_arg;
			argc++;
      		while(!isspace((int)*cur_arg) && *cur_arg) cur_arg++;
			if(*cur_arg) {
				*cur_arg = '\0';
				cur_arg = strip_space(cur_arg + 1);
			} else *cur_arg = '\0';
		...
		if(!cur_cmd->name) PRINTF("%s: Unknown command.\n", argv[0]);
	}

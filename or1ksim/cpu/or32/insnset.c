...
INSTRUCTION (l_j) {
	...
	if(a) {
		...
		// Print hex after program is executed
		char *filename = NULL;
		int cnt;
		cnt = asprintf(&filename, "%s", project_name);
		int length = strlen(filename);
		filename[length - 5] = '\0';
		cnt = asprintf(&filename, "%s.out\0", filename);
	
		autodumpmemory(filename);
	
		char mode[] = "0777";
		int mode_int = strtol(mode, 0, 8);
		chmod(filename, mode_int);
		sim_done();
	}
	/***********************/
}

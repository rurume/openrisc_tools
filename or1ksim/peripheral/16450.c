...
send_char (struct dev_16450 *uart, int bits_send)
{
  ...
  if (uart->regs.mcr & UART_MCR_LOOP)
	...
	{	
	  ...
	  char *output_str = (char*)malloc(sizeof(char) * 2);
	  char *reply = (char *)malloc(sizeof(char) * 4);
	  output_str[0] = buffer[0];
	  output_str[1] = '\0';
	  
	  while(1) {
		int cnt = write(server_sock, output_str, 2);
		read(server_sock, reply, sizeof(char) * 4);
		if(!strcmp(reply, "ACK")) break;
		memset(reply, 0, 4);
	  }
	  memset(output_str, 0, 2);

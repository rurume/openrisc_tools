...
/********* Added by iceobelisk ****************/
void autodumpmemory(char *filename) {
	int length = strlen(filename);
	char *cmd = (char *)calloc(length + 15, sizeof(char));
	sprintf(cmd, "dh 0 262144 > %s", filename);
	
	handle_sim_command_simplified("stall");;
	handle_sim_command_simplified(cmd);
	handle_sim_command_simplified("unstall");
}
/************************************************/
